...
namespace ConnectionManager
{
    public class SocketManager
    {
        ...
        public void init(int portID, int maxConnections, int connectionsPerIP, IDataParser parser,  bool disableNaglesAlgorithm)
        {
            ...
            log.Info("Configurado: GameSocketManage com sucesso na porta (" + portID + ")!");
            log.Info("As conexões máximas por IP foram configuradas para [" + connectionsPerIP + "]!");
        }
            ...
            if (connectionListener != null)
            {
                if (_acceptConnections)
                {
                    try
                    {
                        ...
                        {
                            log.Info("Conexão negada de [" + replyFromComputer.RemoteEndPoint.ToString().Split(':')[0] + "]. Muitas conexões (" + ConnectionCount + ").");
                        }
                    }
                    catch
                    {
