...
namespace Plus.Communication.Packets.Incoming.Marketplace
{
    class BuyOfferEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            {
                Session.SendNotification("Opa, Essa oferta não está mais disponível.");
                this.ReloadOffers(Session);
                return;
            }
            ...
            {
                Session.SendNotification("Opa, esta oferta expirou.");
                this.ReloadOffers(Session);
                return;
            }
            ...
            {
                Session.SendNotification("Item já não está mais no hotel.");
                this.ReloadOffers(Session);
                return;
            }
            else
            {
                ...
                {
                    Session.SendNotification("Para evitar o preço médio, você não pode comprar suas próprias ofertas de mercado.");
                    return;
                }

                if (Convert.ToInt32(Row["total_price"]) > Session.GetHabbo().Credits)
                {
                    Session.SendNotification("Opa, você não tem créditos suficientes para isso.");
                    return;
                }
