...
namespace Plus.Communication.Packets.Incoming.Users
{
    class SetRelationshipEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            {
                Session.SendMessage(new BroadcastMessageAlertComposer("Opa, you can only set a relationship where a friendship exists."));
                return;
            }

            if (Type < 0 || Type > 3)
            {
                Session.SendMessage(new BroadcastMessageAlertComposer("Opa, you've chosen an invalid relationship type."));
                return;
            }
