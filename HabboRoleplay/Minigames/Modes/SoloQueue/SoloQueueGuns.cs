...
{
    public class SoloQueueGuns : IGame
    {
        ...
        public SoloQueueGuns(int maxPlayers, GameMode gameMode)
        {
                    ...
                    foreach (int playerid in Players)
                    {
                        ...
                        new Thread(() =>
                        {
                            while (Counter > 0)
                            {
                                ...
                                {
                                    if (Counter == 11)
                                        Player.SendWhisper("[SoloQueue com Armas] vai começar em alguns segundos!", 1);
                                    else
                                ...
                                if (Counter == 0)
                                {
                                    if (Player != null)
                                    {
                                        ...
                                        {
                                            Player.SendWhisper("Evento Iniciado! Mete bala!", 1);
        ...
        public void Winner()
        {
            try
            {
                ...
                if (Player != null)
                {
                    if (Player.GetRoleplay() != null)
                        Player.GetRoleplay().ReplenishStats(true);
                    Player.SendNotification("Parabéns! Você ganhou o [SoloQueue] com armas!");
                    ...
                    if (Player.GetHabbo() != null && Player.GetHabbo().CurrentRoom != null && Player.GetHabbo().CurrentRoom.GetRoomUserManager() != null && Player.GetHabbo().CurrentRoom.GetRoomUserManager().GetRoomUsers() != null)
                    {
                        lock (Player.GetHabbo().CurrentRoom.GetRoomUserManager().GetRoomUsers())
                        {
                            foreach (var User in Player.GetHabbo().CurrentRoom.GetRoomUserManager().GetRoomUsers())
                            {
                                ...
                                User.GetClient().SendWhisper("[Alerta de EVENTO] " + Player.GetHabbo().Username + " acabou de ganhar o evento [SoloQueue com armas]! Parabéns", 33);
                            }
                        }
                    }
                }
